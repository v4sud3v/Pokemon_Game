...
std::vector<std::unique_ptr<Pokemon>> create_pokemon_roster() {
    ...
    roster.push_back(std::make_unique<Pokemon>(
        "Charmander", 95, 35, 8,
        Moves{{"Ember", 40, "Fire"}, {"Scratch", 30, "Normal"}}
    ));
    
    roster.push_back(std::make_unique<Pokemon>(
        "Squirtle", 110, 28, 12,
        Moves{{"Water Gun", 38, "Water"}, {"Tackle", 30, "Normal"}}
    ));
...
void battle(Pokemon& player, Pokemon& opponent) {
    ...
    while (!player.is_fainted() && !opponent.is_fainted()) {
        ...
        player.use_attack(opponent, move_choice);
        ...
        opponent.use_attack(player, opponent_move);
